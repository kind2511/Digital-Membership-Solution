...
function MedlemsNivaer() {
    ...
    return (
        <div className="medlems-nivaer-container">
            <h2 className="section-title">Medlems Nivåer</h2>
                ...
                <div className="level-list">
                    {levels.map((level, index) => (
                        <div key={index} className="level-item">
                            ...
                            <div>
                                <button className="level-button edit-button" onClick={() => handleEditLevel(level.levelID)}>Edit</button>
                                <button className="level-button delete-button" onClick={() => handleDeleteLevel(level.levelID)}>Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
